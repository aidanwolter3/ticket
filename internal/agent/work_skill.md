Find and execute the next available ticket using the workflow below.
...
If the output is `null`, tell the user there is no available work and stop.

Otherwise, read the result object. It has this shape:
...
`type` is one of:
- `new_work` — a fresh ticket to implement from scratch
- `amendment` — an in-review ticket with reviewer threads to address

`ticket.worktree_path` may be empty if worktrees are disabled.

Use `ticket peek-work --json` to view claimable work without claiming it.

### 2. Enter worktree (if provided)

If `worktree_path` is non-empty, call:
...
3. Commit **all changes for this task in a single commit**:
   - Commit on the feature branch (already set up in the worktree if `worktree_path` was provided).
